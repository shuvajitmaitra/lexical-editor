...
import { $getSelection, $isLineBreakNode, $isRangeSelection, CLICK_COMMAND, COMMAND_PRIORITY_CRITICAL, COMMAND_PRIORITY_HIGH, COMMAND_PRIORITY_LOW, KEY_ESCAPE_COMMAND, SELECTION_CHANGE_COMMAND, } from 'lexical';
...
function FloatingLinkEditor({ editor, isLink, setIsLink, anchorElem, isLinkEditMode, setIsLinkEditMode, }) {
    ...
    return (<div ref={editorRef} className="absolute left-0 top-0 w-full max-w-sm rounded-md opacity-0 shadow-md">
      {!isLink ? null : isLinkEditMode ? (<div className="flex items-center space-x-2 rounded-md border p-1 pl-2">
        ...
        </div>) : (<div className="flex items-center justify-between rounded-md border p-1 pl-2">
          <a href={sanitizeUrl(linkUrl)} target="_blank" rel="noopener noreferrer" className="overflow-hidden text-ellipsis whitespace-nowrap text-sm">
            {linkUrl}
          </a>
          <div className="flex">
            <Button size="icon" variant="ghost" onClick={() => {
...
function useFloatingLinkEditorToolbar(editor, anchorElem, isLinkEditMode, setIsLinkEditMode) {
    ...
    return createPortal(<FloatingLinkEditor editor={activeEditor} isLink={isLink} anchorElem={anchorElem} setIsLink={setIsLink} isLinkEditMode={isLinkEditMode} setIsLinkEditMode={setIsLinkEditMode}/>, anchorElem);
}
...
//# sourceMappingURL=floating-link-editor-plugin.jsx.map