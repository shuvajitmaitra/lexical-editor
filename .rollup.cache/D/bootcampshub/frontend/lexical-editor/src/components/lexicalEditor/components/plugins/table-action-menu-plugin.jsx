...
function TableActionMenu({ onClose, tableCellNode: _tableCellNode, setIsMenuOpen, contextRef, cellMerge, }) {
    ...
    if (cellMerge) {
        if (canMergeCells) {
            mergeCellButton = (<CommandItem onSelect={() => mergeTableCellsAtSelection()}>
          Merge cells
        </CommandItem>);
        }
        else if (canUnmergeCell) {
            mergeCellButton = (<CommandItem onSelect={() => unmergeTableCellsAtSelection()}>
          Unmerge cells
        </CommandItem>);
        }
    }
        ...
        <CommandList>
          <CommandGroup>
            ...
            <CommandItem className="flex justify-between">
              Background color
              <ColorPicker color={backgroundColor} onChange={handleCellBackgroundColor} icon={<PaintBucketIcon className="size-4"/>}/>
            </CommandItem>
...
function TableCellActionMenuContainer({ anchorElem, cellMerge, }) {
    ...
    return (<div className="table-cell-action-button-container absolute top-0 will-change-transform" ref={menuButtonRef}>
      {tableCellNode != null && (<Popover open={isMenuOpen} onOpenChange={setIsMenuOpen}>
          <PopoverTrigger asChild>
            <button type="button" className="relative inline-block flex cursor-pointer items-center justify-center bg-none" onClick={(e) => {
                e.stopPropagation();
          ...
          <PopoverContent className="w-[200px] p-0">
            <TableActionMenu contextRef={menuRootRef} setIsMenuOpen={setIsMenuOpen} onClose={() => setIsMenuOpen(false)} tableCellNode={tableCellNode} cellMerge={cellMerge}/>
          </PopoverContent>
        </Popover>)}
    </div>);
}
export function TableActionMenuPlugin({ anchorElem, cellMerge = false, }) {
...
//# sourceMappingURL=table-action-menu-plugin.jsx.map