...
export default function BeautifulMentionComponent(props) {
    ...
    if (Component) {
        return (<Component 
        // @ts-ignore
        ref={ref} trigger={trigger} value={value} data={data} className={composedClassNames} data-beautiful-mention={mention}>
        {mention}
      </Component>);
    }
    if (classNames) {
        return (<span ref={ref} className={isSelected && !!classNames.containerFocused
                ? classNames.containerFocused
                : classNames.container} data-beautiful-mention={mention}>
        <span className={classNames.trigger}>{trigger}</span>
        <span className={classNames.value}>{value}</span>
      </span>);
    }
    return (<span ref={ref} className={composedClassNames} data-beautiful-mention={mention}>
      {mention}
    </span>);
}
//# sourceMappingURL=MentionComponent.jsx.map