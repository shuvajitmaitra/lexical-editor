...
export function BeautifulMentionsPlugin(props) {
    ...
    if (props.combobox) {
        return (<ComboboxPlugin options={options} loading={loading} onQueryChange={setQueryString} onSelectOption={handleSelectOption} onReset={() => {
                setTrigger(null);
    ...
    return (<LexicalTypeaheadMenuPlugin commandPriority={COMMAND_PRIORITY_NORMAL} onQueryChange={setQueryString} onSelectOption={handleSelectMenuItem} triggerFn={checkForMentionMatch} options={options} anchorClassName={menuAnchorClassName} onClose={handleClose} menuRenderFn={(anchorElementRef, { selectedIndex, selectOptionAndCleanUp, setHighlightedIndex }) => {
            ...
            if (anchorElementRef.current &&
                ...
                isEditorFocused &&
                EmptyComponent) {
                return ReactDOM.createPortal(<EmptyComponent />, anchorElementRef.current);
            }
            return anchorElementRef.current && open
                ? ReactDOM.createPortal(<MenuComponent loading={loading} role="menu" aria-label="Choose a mention" aria-hidden={!open} aria-activedescendant={!IS_MOBILE &&
                        selectedIndex !== null &&
                        !!options[selectedIndex]
                        ? options[selectedIndex].displayValue
                        : ""}>
                {options.map((option, i) => (<MenuItemComponent key={option.key} tabIndex={-1} selected={!IS_MOBILE && selectedIndex === i} ref={option.setRefElement} role="menuitem" aria-selected={!IS_MOBILE && selectedIndex === i} aria-label={`Choose ${option.value}`} item={option.menuItem} itemValue={option.value} label={option.displayValue} {...option.data} onClick={() => {
              ...
              </MenuComponent>, anchorElementRef.current)
                : null;
        }}/>);
}
//# sourceMappingURL=BeautifulMentionsPlugin.jsx.map