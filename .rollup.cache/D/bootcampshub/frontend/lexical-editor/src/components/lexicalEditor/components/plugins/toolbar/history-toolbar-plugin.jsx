...
export function HistoryToolbarPlugin() {
    ...
    return (<div className="flex items-center gap-1">
      <Button disabled={!canUndo || !isEditable} onClick={() => {
            activeEditor.dispatchCommand(UNDO_COMMAND, undefined);
        }} title={IS_APPLE ? 'Undo (⌘Z)' : 'Undo (Ctrl+Z)'} type="button" aria-label="Undo" size="sm" className="h-8 w-8" variant={'outline'}>
        <UndoIcon className="size-4"/>
      </Button>
      <Button disabled={!canRedo || !isEditable} onClick={() => {
            activeEditor.dispatchCommand(REDO_COMMAND, undefined);
        }} title={IS_APPLE ? 'Redo (⇧⌘Z)' : 'Redo (Ctrl+Y)'} type="button" aria-label="Redo" size="sm" className="h-8 w-8" variant={'outline'}>
        <RedoIcon className="size-4"/>
      </Button>
    </div>);
}
//# sourceMappingURL=history-toolbar-plugin.jsx.map