...
import { $getSelection, $isParagraphNode, $isRangeSelection, $isTextNode, COMMAND_PRIORITY_LOW, FORMAT_TEXT_COMMAND, SELECTION_CHANGE_COMMAND, createCommand, } from 'lexical';
import { BoldIcon, ItalicIcon, StrikethroughIcon, UnderlineIcon, CodeIcon, LinkIcon, SubscriptIcon, SuperscriptIcon, ChevronDown, Sparkles, MessageSquare, AlignJustify, Scissors, Maximize, Minimize, FileDigit, PenTool, Languages, Edit, Loader2, } from 'lucide-react';
import { createPortal } from 'react-dom';
import { Separator } from '../../ui/separator';
import { ToggleGroup, ToggleGroupItem } from '../../ui/toggle-group';
import { DropdownMenu, DropdownMenuContent, DropdownMenuItem, DropdownMenuSeparator, DropdownMenuTrigger, DropdownMenuGroup, DropdownMenuLabel, DropdownMenuSub, DropdownMenuSubContent, DropdownMenuSubTrigger, } from '../../ui/dropdown-menu';
...
function AILoadingIndicator({ visible }) {
    if (!visible)
        return null;
    return (<div className="fixed inset-0 flex items-center justify-center bg-black/10 z-50">
...
const AI_TEXT_TRANSFORM_COMMAND = createCommand('AI_TEXT_TRANSFORM_COMMAND');
// Helper function to process AI transformation asynchronously
async function processAITransformation(payload, selectedText, setIsAILoading, onAIGeneration) {
...
function FloatingTextFormat({ editor, anchorElem, isLink, isBold, isItalic, isUnderline, isCode, isStrikethrough, isSubscript, isSuperscript, setIsLinkEditMode, editorRef, setIsAILoading, onAIGeneration, }) {
    ...
    const transformText = useCallback((transformType) => {
    ...
    return (<div ref={popupCharStylesEditorRef} className="absolute left-0 top-0 z-10 flex gap-1 rounded-md border p-1 opacity-0 shadow-md transition-opacity duration-300 will-change-transform bg-background">
      {editor.isEditable() && (<>
          <DropdownMenu>
          ...
          <ToggleGroup type="multiple" defaultValue={[
            ...
            <ToggleGroupItem value="strikethrough" aria-label="Toggle strikethrough" onClick={() => {
          ...
          <ToggleGroup type="single" defaultValue={isSubscript ? 'subscript' : isSuperscript ? 'superscript' : ''}>
            <ToggleGroupItem value="subscript" aria-label="Toggle subscript" onClick={() => {
                editor.dispatchCommand(FORMAT_TEXT_COMMAND, 'subscript');
            }} size="sm">
              <SubscriptIcon className="h-4 w-4"/>
            </ToggleGroupItem>
            <ToggleGroupItem value="superscript" aria-label="Toggle superscript" onClick={() => {
...
function useFloatingTextFormatToolbar(editor, anchorElem, setIsLinkEditMode, editorRef, setIsAILoading, onAIGeneration) {
    ...
    useEffect(() => {
        ...
        editor.registerCommand(AI_TEXT_TRANSFORM_COMMAND, (payload) => {
    ...
    return createPortal(<FloatingTextFormat editor={editor} anchorElem={anchorElem} isLink={isLink} isBold={isBold} isItalic={isItalic} isStrikethrough={isStrikethrough} isSubscript={isSubscript} isSuperscript={isSuperscript} isUnderline={isUnderline} isCode={isCode} setIsLinkEditMode={setIsLinkEditMode} editorRef={editorRef} setIsAILoading={setIsAILoading} onAIGeneration={onAIGeneration}/>, anchorElem);
}
export function FloatingTextFormatToolbarPlugin({ anchorElem, onAIGeneration, }) {
    ...
    return (<>
      {useFloatingTextFormatToolbar(editor, anchorElem, setIsLinkEditMode, editorRef, setIsAILoading, onAIGeneration)}
      <AILoadingIndicator visible={isAILoading}/>
    </>);
}
//# sourceMappingURL=floating-text-format-plugin.jsx.map