 ...
 * React Component
 * ------------------------------------------------------------------------- */
const LexicalJsonRenderer = ({ lexicalState, showTOC = false, className = '', }) => {
    ...
    return (<div ref={containerRef} className={`lexical-rendered-content ${className}`} data-lexical-renderer="true"/>);
};
 ...
 * ------------------------------------------------------------------------- */
function generateHtmlAndTOC(lexicalState) {
    var _a;
    const contentContainer = document.createElement('div');
    contentContainer.className = 'lexical-content';
    const headings = [];
    if ((_a = lexicalState.root) === null || _a === void 0 ? void 0 : _a.children) {
 ...
 * ------------------------------------------------------------------------- */
function processNode(node, headings) {
    if (!node || !node.type)
        return null;
    // Handle text nodes
    if (node.type === 'text') {
        ...
        if (node.format) {
            ...
            if (node.format & 1)
                span.classList.add('font-bold'); // Bold
            if (node.format & 2)
                span.classList.add('italic'); // Italic
            if (node.format & 4)
                span.classList.add('underline'); // Underline
            if (node.format & 8)
                span.classList.add('line-through'); // Strikethrough
            if (node.format & 16)
                span.classList.add('text-xs', 'align-super'); // Superscript
            if (node.format & 32)
                span.classList.add('text-xs', 'align-sub'); // Subscript
            if (node.format & 64)
                span.classList.add('font-mono', 'bg-gray-100', 'px-1', 'rounded'); // Code
        }
 ...
 * createParagraph
 * ------------------------------------------------------------------------- */
function createParagraph(node, headings) {
 ...
 * createHeading
 * ------------------------------------------------------------------------- */
function createHeading(node, headings) {
 ...
 * createList
 * ------------------------------------------------------------------------- */
function createList(node, headings) {
 ...
 * createListItem
 * ------------------------------------------------------------------------- */
function createListItem(node, headings) {
 ...
 * createCheckList
 * ------------------------------------------------------------------------- */
function createCheckList(node, headings) {
 ...
 * createCheckListItem
 * ------------------------------------------------------------------------- */
function createCheckListItem(node, headings) {
    const item = document.createElement('li');
 ...
 * createQuote
 * ------------------------------------------------------------------------- */
function createQuote(node, headings) {
 ...
 * createCode
 * ------------------------------------------------------------------------- */
function createCode(node, headings) {
 ...
 * createLink
 * ------------------------------------------------------------------------- */
function createLink(node, headings) {
    const link = document.createElement('a');
    link.href = node.url || '#';
    link.className = 'text-blue-600 hover:underline';
...
function createImage(node) {
    ...
    if (node.alignment) {
        ...
        else if (node.alignment === 'start' ||
            node.alignment === 'left') {
            figure.classList.add('float-left', 'mr-4');
        }
        else if (node.alignment === 'end' ||
 ...
 * ------------------------------------------------------------------------- */
function createInlineImage(node) {
    const figure = document.createElement('figure');
    figure.className = 'my-4';
    const img = document.createElement('img');
    img.src = node.src || '';
 ...
 * ------------------------------------------------------------------------- */
function createEmbedElement(node, type) {
 ...
 * createTable
 * ------------------------------------------------------------------------- */
function createTable(node, headings) {
    ...
    if (node.children) {
        ...
        const hasHeaderRows = node.children.some((row) => row.children &&
            row.children.some((cell) => cell.headerState === 1 || cell.headerState === 3));
        if (hasHeaderRows) {
            ...
            node.children.forEach((rowNode) => {
                ...
                const isHeaderRow = rowNode.children &&
                    rowNode.children.some((cell) => cell.headerState === 1 || cell.headerState === 3);
                if (isHeaderRow) {
 ...
 * createTableRow
 * ------------------------------------------------------------------------- */
function createTableRow(node, headings) {
...
function createTableCell(node) {
    var _a;
    const headerState = (_a = node.headerState) !== null && _a !== void 0 ? _a : 0;
 ...
 * createCollapsible
 * ------------------------------------------------------------------------- */
function createCollapsible(node, headings) {
 ...
 * ------------------------------------------------------------------------- */
function createCollapsibleTitle(node, headings) {
 ...
 * ------------------------------------------------------------------------- */
function createCollapsibleContent(node, headings) {
    const content = document.createElement('div');
 ...
 * ------------------------------------------------------------------------- */
function createExcalidraw(node) {
    var _a;
    ...
    try {
        ...
        const bgColor = ((_a = excalidrawData.appState) === null || _a === void 0 ? void 0 : _a.viewBackgroundColor) || '#fff';
          ...
          >
            ...
            if (el.type === 'ellipse') {
                ...
                const rotation = el.angle
                    ? `rotate(${(el.angle * 180) / Math.PI}, ${el.x + el.width / 2}, ${el.y + el.height / 2})`
                    : '';
                return `
                    <rect
                      x="${el.x}"
...
function createPoll(node) {
    ...
    if (node.options && Array.isArray(node.options)) {
        ...
        form.addEventListener('submit', (e) => {
            e.preventDefault();
            const selected = form.querySelector('input[name="poll-option"]:checked');
            if (selected) {
 ...
 * createColumnsLayout
 * ------------------------------------------------------------------------- */
function createColumnsLayout(node, headings) {
 ...
 * createColumn
 * ------------------------------------------------------------------------- */
function createColumn(node, headings) {
...
function createMention(node) {
    ...
    const userId = (_b = (_a = node.data) === null || _a === void 0 ? void 0 : _a.id) !== null && _b !== void 0 ? _b : 'N/A';
    const avatarUrl = (_d = (_c = node.data) === null || _c === void 0 ? void 0 : _c.avatarUrl) !== null && _d !== void 0 ? _d : 'https://placehold.co/400';
    const userBio = (_f = (_e = node.data) === null || _e === void 0 ? void 0 : _e.bio) !== null && _f !== void 0 ? _f : 'Lorem ipsum dolor sit amet, consectetur adipiscing elit.';
 ...
 * mapColorToTailwind
 * ------------------------------------------------------------------------- */
function mapColorToTailwind(color) {
    const colorMap = {
 ...
 * ------------------------------------------------------------------------- */
function mapBgColorToTailwind(color) {
    const bgColorMap = {
        'rgb(0, 0, 0)': 'bg-black',
        black: 'bg-black',
 ...
 * ------------------------------------------------------------------------- */
function mapFontSizeToTailwind(fontSize) {
    const fontSizeMap = {
        '12px': 'text-xs',
        '14px': 'text-sm',
        '16px': 'text-base',
 ...
 * returns a random ID.
 * ------------------------------------------------------------------------- */
function createUniqueHeadingId(text) {
    const slug = text
        .toLowerCase()
...
//# sourceMappingURL=JsonRenderer.jsx.map