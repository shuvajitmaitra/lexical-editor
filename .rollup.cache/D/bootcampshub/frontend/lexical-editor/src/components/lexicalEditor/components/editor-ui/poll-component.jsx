...
function PollOptionComponent({ option, index, options, totalVotes, withPollNode, }) {
    ...
    return (<div className="mb-2.5 flex flex-row items-center">
      <div className={`relative mr-2.5 flex h-[22px] w-[22px] rounded-md border border-gray-400 ${checked
            ...
            : ''}`}>
        <input ref={checkboxRef} className="absolute block h-full w-full cursor-pointer border-0 opacity-0" type="checkbox" onChange={(e) => {
            withPollNode((node) => {
      ...
      <div className="relative flex flex-[10px] cursor-pointer overflow-hidden rounded-md border border-primary">
        <div className="transition-width absolute left-0 top-0 z-0 h-full bg-accent duration-1000 ease-in-out" style={{ width: `${votes === 0 ? 0 : (votes / totalVotes) * 100}%` }}/>
      ...
      <button disabled={options.length < 3} className={`relative z-0 ml-1.5 flex h-7 w-7 cursor-pointer rounded-md border-0 bg-transparent bg-[position:6px_6px] bg-no-repeat opacity-30 before:absolute before:left-[13px] before:top-1.5 before:block before:h-[15px] before:w-0.5 before:-rotate-45 before:bg-gray-400 before:content-[''] after:absolute after:left-[13px] after:top-1.5 after:block after:h-[15px] after:w-0.5 after:rotate-45 after:bg-gray-400 after:content-[''] hover:bg-gray-100 hover:opacity-100 disabled:cursor-not-allowed disabled:hover:bg-transparent disabled:hover:opacity-30`} aria-label="Remove" onClick={() => {
            withPollNode((node) => {
...
export default function PollComponent({ question, options, nodeKey, }) {
    ...
    return (<div className={`min-w-[400px] max-w-[600px] cursor-pointer select-none rounded-lg border border-gray-200 bg-background ${isFocused ? 'outline outline-2 outline-primary' : ''}`} ref={ref}>
...
//# sourceMappingURL=poll-component.jsx.map