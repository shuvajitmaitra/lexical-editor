...
function LazyImage({ altText, className, imageRef, src, width, height, position, }) {
    ...
    return (<img className={className || undefined} src={src} alt={altText} ref={imageRef} data-position={position} style={{
            display: 'block',
            height,
            width,
        }} draggable="false"/>);
    ...
    return (<Suspense fallback={null}>
      <>
        <span draggable={draggable}>
          {isEditable && (<Button className="image-edit-button absolute right-1 top-1" variant="outline" ref={buttonRef} onClick={() => {
                showModal('Update Inline Image', (onClose) => (<UpdateInlineImageDialog activeEditor={editor} nodeKey={nodeKey} onClose={onClose}/>));
            }}>
              Edit
            </Button>)}
          <LazyImage className={`max-w-full cursor-default ${isFocused
        ...
        {showCaption && (<div className="image-caption-container absolute bottom-1 left-0 right-0 m-0 block min-w-[100px] overflow-hidden border-t bg-white/90 p-0">
            <LexicalNestedComposer initialEditor={caption}>
              <AutoFocusPlugin />
              <LinkPlugin />
              <RichTextPlugin contentEditable={<ContentEditable placeholder="Enter a caption..." className="ImageNode__contentEditable user-select-text word-break-break-word relative block min-h-5 w-[calc(100%-20px)] cursor-text resize-none whitespace-pre-wrap border-0 p-2.5 text-sm caret-primary outline-none" placeholderClassName="ImageNode__placeholder text-sm text-muted-foreground overflow-hidden absolute top-2.5 left-2.5 pointer-events-none text-ellipsis user-select-none whitespace-nowrap inline-block"/>} ErrorBoundary={LexicalErrorBoundary}/>
...
//# sourceMappingURL=inline-image-component.jsx.map