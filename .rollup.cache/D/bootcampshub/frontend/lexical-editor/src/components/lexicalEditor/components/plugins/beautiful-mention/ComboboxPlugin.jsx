...
export function ComboboxPlugin(props) {
    ...
    return ReactDOM.createPortal(<ComboboxComponent loading={loading} itemType={itemType} role="menu" aria-activedescendant={selectedIndex !== null && !!options[selectedIndex]
            ? options[selectedIndex].displayValue
            : ""} aria-label={"Choose trigger and value"} aria-hidden={!open}>
      {options.map((option, index) => (<ComboboxItemComponent key={option.key} selected={index === selectedIndex} role="menuitem" aria-selected={selectedIndex === index} aria-label={`Choose ${option.value}`} item={option.comboboxItem} ref={option.setRefElement} onClick={() => {
                handleClick(index);
            }} onMouseEnter={() => {
    ...
    </ComboboxComponent>, anchor);
}
//# sourceMappingURL=ComboboxPlugin.jsx.map