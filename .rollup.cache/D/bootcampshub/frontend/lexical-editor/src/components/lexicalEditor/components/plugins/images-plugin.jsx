...
import { $createParagraphNode, $createRangeSelection, $getSelection, $insertNodes, $isNodeSelection, $isRootOrShadowRoot, $setSelection, COMMAND_PRIORITY_EDITOR, COMMAND_PRIORITY_HIGH, COMMAND_PRIORITY_LOW, DRAGOVER_COMMAND, DRAGSTART_COMMAND, DROP_COMMAND, createCommand, } from 'lexical';
 ...
 */
async function defaultUploadImageToServer(file) {
    try {
...
export function InsertImageUploadedDialogBody({ onClick, onClose, onImageUpload, }) {
    ...
    return (<div className="grid gap-4 py-4">
      <div className="grid gap-2">
        ...
        <Input id="image-upload" type="file" onChange={(e) => handleFileChange(e.target.files)} accept="image/*" data-test-id="image-modal-file-upload"/>
      </div>
      
      {previewSrc && (<div className="grid place-items-center">
          <img src={previewSrc} alt="Preview" className="max-h-48 max-w-full object-contain rounded-md border border-gray-200"/>
        </div>)}
      ...
      {uploadError && (<div className="text-red-500 text-sm p-2 bg-red-50 rounded">{uploadError}</div>)}
      
      <DialogFooter>
        <Button type="submit" disabled={isDisabled} onClick={handleUpload} data-test-id="image-modal-file-upload-btn">
          {isUploading ? (<span className="flex items-center gap-2">
...
export function InsertImageDialog({ activeEditor, onClose, onImageUpload, }) {
    ...
    return (<Tabs defaultValue="url">
      ...
      <TabsContent value="file">
        <InsertImageUploadedDialogBody onClick={onClick} onClose={onClose} onImageUpload={onImageUpload}/>
      </TabsContent>
    </Tabs>);
}
...
//# sourceMappingURL=images-plugin.jsx.map